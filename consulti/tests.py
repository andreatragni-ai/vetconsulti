...
@pytest.mark.django_db
def test_eco_richiede_il_referto_e_avvisa_sulle_proiezioni(richiedente, refertatore):
    ...
    r = _richiesta(richiedente, refertatore, tipo=TipoEsame.ECO)
    # Blocca solo il referto dell'ecografo; le proiezioni avvisano (24/09/2026).
    motivo = regole.perche_non_puoi_inviare(r)
    assert 'ecografo' in motivo and 'Quattro camere' not in motivo
    assert [e.etichetta for e in regole.consigliati_mancanti(r)] == ['Quattro camere']
    ...
    assert regole.consigliati_mancanti(r) == []
...
def test_elementi_obbligatori_sono_la_stessa_regola_della_frase(richiedente, refertatore):
    ...
    assert [e.blocca for e in elementi] == [True, False, False]
    assert not any(e.fatto for e in elementi)
    # La frase del blocco parla solo di cio' che blocca; il resto e' l'avviso.
    assert regole.allegati_mancanti(r) == [elementi[0].frase]
    assert regole.perche_non_puoi_inviare(r) == f'Manca {elementi[0].frase}.'
    assert [e.etichetta for e in regole.consigliati_mancanti(r)] == ['Asse corto', 'Quattro camere']
    _allega(r, CategoriaAllegato.ECO_REFERTO_PDF)
    clip = _allega(r, CategoriaAllegato.ECO_CLIP, 'c.mp4', b'mp4')
